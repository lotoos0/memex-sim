...
try {
  ...
  $codePath = Join-Path $root "scripts/qa/sim-dead-floor-test.run.js"
  ...
  if ($outputText -match "### Error") {
    throw "Deterministic sim test failed."
  }
  if ($outputText -notmatch '"ok"\s*:\s*true') {
    throw "Deterministic sim test did not return ok=true."
  }

  Write-Host "PASS: deterministic DEAD floor test"
} finally {
